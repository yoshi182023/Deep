...
async def main():
    try:
        ...
        # FILL_THIS_IN: Initialize the MCP client with server configuration
        # Hint: Use MultiServerMCPClient with a dictionary containing:
        #   - A key for the server name (e.g., "task-manager")
        #   - A nested dictionary with "transport", "command", and "args" keys
        #   - transport should be "stdio"
        #   - command should be the path to your Python interpreter
        #   - args should be a list with the path to task_mcp_server.py
        client = None

        # FILL_THIS_IN: Get the tools from the MCP client
        # Hint: Use await client.get_tools()
        tools = None

        # Initialize Ollama chat model explicitly
        model = init_chat_model("gemma4:e4b-it-q4_k_M", model_provider="ollama")

        # FILL_THIS_IN: Create an agent with the model and tools
        # Hint: Use create_agent(model, tools)
        agent = None

        print("Ready! Type your message (or 'quit'/'exit'/'q' to end):\n")

        # FILL_THIS_IN: Initialize an empty list to maintain conversation history
        # Hint: This will store HumanMessage and AIMessage objects
        messages = None
        ...
        while True:
            try:
                ...
                # FILL_THIS_IN: Add user message to history
                # Hint: Create a HumanMessage with the user_input content and append to messages
                pass
                    ...
                    # Check for "messages" key
                        ...
                        if len(response_messages) > existing_message_count:
                            ...
                            if assistant_messages:
                                ...
                                # FILL_THIS_IN: Add the new assistant message to history
                                # Hint: Append assistant_messages[-1] to messages
                                pass
                        else:
